...
int main(int argc, char *argv[])
{
	...
	for (i=0;i<4;i++)
	{
     ...
     while (1)
     {
        ...
        if (fsmServer==0)
        {
          ...
          printf("sending [%s] to %s on port %d\n", reply, tcpClients[0].ipAddress, tcpClients[0].port);
          for (int i = 0; i < 8; i++){
          ...
          printf("sending [%s] to %s on port %d\n", reply, tcpClients[1].ipAddress, tcpClients[1].port);
          for (int i = 0; i < 8; i++){
          ...
          printf("sending [%s] to %s on port %d\n", reply, tcpClients[2].ipAddress, tcpClients[2].port);
          for (int i = 0; i < 8; i++){
          ...
          printf("sending [%s] to %s on port %d\n", reply, tcpClients[3].ipAddress, tcpClients[3].port);
          for (int i = 0; i < 8; i++){
    ...
    int lineToExamine;
        ...
        while (token != NULL){
          if (w != 0){
            receivedDataClient[w-1] = atoi(token);
          }
          token = strtok(NULL, " \n");
          w++;
        }

        if (receivedDataClient[1] == deck[12]){
          sprintf(reply, "%s a gagné la partie!\nCoupable : %s\n", tcpClients[receivedDataClient[0]].name, nomcartes[12]);
          broadcastMessage(reply);
          printf("broadcasting [%s]\n", reply);
          fsmServer = 0;
        }
        else{
          sprintf(reply, "%s accuse %s à tort\n", tcpClients[receivedDataClient[0]].name, nomcartes[receivedDataClient[1]]);
          broadcastMessage(reply);
          printf("broadcasting [%s]\n", reply);
        }
        ...
        colToExamine = receivedDataClient[1];
        playerDataToHide = receivedDataClient[0];

        sprintf(reply, "%s demande qui a des symboles %d", tcpClients[playerDataToHide].name, colToExamine);
        broadcastMessage(reply);
        printf("broadcasting [%s]\n", reply);

        for (int i = 0; i < 4; i++){
          if (i != playerDataToHide){
            int dataTable;
            if (tableCartes[i][colToExamine] != 0){
              dataTable = 100;
            }
            else {dataTable = 0;}
            sprintf(reply, "V %d %d %d", i, colToExamine, dataTable);
            broadcastMessage(reply);
            printf("broadcasting [%s]\n", reply);
          }
        }
        ...
        colToExamine = receivedDataClient[2];
        lineToExamine = receivedDataClient[1];
        sprintf(reply, "V %d %d %d", lineToExamine, colToExamine, tableCartes[lineToExamine][colToExamine]);
        broadcastMessage(reply);
        printf("broadcasting [%s]\n", reply);
    ...
    if (fsmServer != 0){
      ...
      sprintf(reply, "Z %d", joueurCourant);
      broadcastMessage(reply);
      printf("broadcasting [%s]\n", reply);
    }
